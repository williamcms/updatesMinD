	...
	var mountHTML = $('#generateHTML').on('click', function(){
		...
		if(type == 1){
			...
			$.each($('#banners > .form-group'), function(i){
				bannerimage = $(this).find('input[name=banner-image-'+ i +']').val(),
			...
			console.log(vitrineTemp);
			$('#viewRenderedHTML').html(vitrineTemp)
			$('#viewHTMLCode').text(vitrineTemp)
