...
$(document).ready(function () {
	...
	$(document).on("click", "#product .getDetails", function () {
		let inputN = $(this).data("input"),
			ref = $("input[name=produto-" + inputN + "]").val(),
			product = {};
		...
		$.ajax({
			...
			product = {
				name: $.trim((e.first().productName).slice(0, 40)) + (e.first().productName.length > 40 ? "..." : ""),
				qty: 1,
				instock: e.first().items.first().sellers.first().commertialOffer.AvailableQuantity,
				sku: e.first().items.first().itemId,
				seller: e.first().items.first().sellers.first().sellerId
			};

			if ($(".row.newFields[data-input=" + inputN + "]").length >= 1) {
				$(".row.newFields").find("input[name=product-name-" + inputN + "]").val(product.name);
				$(".row.newFields").find("input[name=product-qty-" + inputN + "]").val(product.qty);
				$(".row.newFields").find("input[name=product-sku-" + inputN + "]").val(product.sku);
				$(".row.newFields").find("input[name=product-qty-instock-" + inputN + "]").val(product.instock);
				$(".row.newFields").find("input[name=product-seller-" + inputN + "]").val(product.seller);
			}
	...
	$("#generateLINK").on("click", function () {
		let cartLink = "https://www.casamind.com.br/checkout/cart/add?";
		$.each($("#product > .form-group"), function (i) {
			let productQty = $(this).find("input[name=product-qty-" + i + "]").val();
			let productSku = $(this).find("input[name=product-sku-" + i + "]").val();
			let productSeller = $(this).find("input[name=product-seller-" + i + "]").val();

			if(productSku == "" || productQty == "" || productSeller == "") return 0;
			cartLink = cartLink + `${(i === 0 ? "" : "&")}sku=${productSku}&qty=${productQty}&seller=${productSeller}&redirect=true&sc=3`;
		});
